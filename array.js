...
const americano = createMenuItem("Americano", 2, "Drinks");
console.log(americano);
const sandwich = createMenuItem("Sandwich", 12, "Breakfast");
console.log(sandwich);
const kimbab = createMenuItem("Kimbab", 10, "Lunch");
console.log(kimbab);
...
function discount(str) {
  ...
  } else if ("public" === str.toLowerCase()) {
...
function newRating(name, rating, feedback) {
  ...
  const newRate = newOne.reduce((acc, [key, value]) => {
    acc[key] = value;
    return acc;
  }, {});
  return newRate;
}
//console.log(newRating("Luce",5,"Absolutely love that they have karaoke Fridays! Food and drink selection is okay."));
...
function getReviewByIndex(reviews, index) {
  /* code here */
  let newStr = "";
  let chosenObj = reviews.filter((el) => {
    return el == reviews[index];
  });
  chosenObj.map((el) => {
    newStr += `${el.name} gave the restaurant a ${el.rating} star review and their feedback was: ${el.feedback}\n`;
  });
  //const {name,rating,feedback}=reviews
  return newStr;
}
...
const newObj = info.reduce((acc, [key, value]) => {
  acc[key] = value;
  return acc;
}, {});
console.log(newObj);
...
function infoMovie(movieDatabase) {
  const { title, duration, stars } = movieDatabase;
  let arr = [title, duration, stars];
  return arr;
}
